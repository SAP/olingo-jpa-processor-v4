...
class FromImpl<Z, X> extends PathImpl<X> implements From<Z, X> {
   ...
   */
  @Override
  public <Y> Join<X, Y> join(@Nonnull final SingularAttribute<? super X, Y> attribute, JoinType jt) {
    throw new NotImplementedException();
  }
  ...
  public <X, Y> Join<X, Y> join(@Nonnull final String attributeName, final JoinType jt) {
    ...
    try {
      ...
      final Optional<JPAAttribute> a = source.getAttribute(Objects.requireNonNull(attributeName));
      final JPAAttribute joinAttribute = a.orElseGet(() -> {
        try {
